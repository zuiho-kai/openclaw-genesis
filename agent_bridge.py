    ...
    if round_num > 1:
        ...
        if has_subs:
            ...
            for need in open_needs_now:
                ...
                if not subs:
                    continue
                msg += f"[{need['id']}] {need['title']}：\n"
                for s in subs:
                    preview = s["content"][:300].replace("\n", " ")
                    msg += f"  · {s['citizen_id']}: {preview}\n"
...
def extract_actions(text):
    ...
    if not actions and text.strip() and len(text.strip()) > 10:
...
def run_citizen_turn(citizen_id, day, round_num=1, total_rounds=3):
    ...
    for action in actions:
        ...
        print(f"  [{citizen_id}] → {action.get('type', '?')}")
